 ...
 * outcomes.
 *
 * Pure server component. The tab interactivity is delegated to a
 * small client island (`PersonasTabs`), which keeps this file a
 * server component while still letting visitors switch between
 * roles.
 *
 * Content is sourced from `@/content/personas.json` so the
                  ...
                  <div className="rounded-2xl border border-brand-500/30 bg-brand-500/5 p-6 backdrop-blur">
                    ...
                    <ul className="mt-4 space-y-3">
                      {persona.outcomes.map((outcome) => (
                        <li
                          key={outcome}
                          className="flex gap-3 text-base text-ink-100"
                        >
                          <span
                            aria-hidden="true"
